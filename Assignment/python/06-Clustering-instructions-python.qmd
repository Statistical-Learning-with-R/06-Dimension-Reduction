...
```{python}
songs = pd.read_csv("https://www.dropbox.com/s/hijzbof7nnche09/top_artists_spotify-no_labels.csv?dl=1")

ss = StandardScalar()
songs_matrix = ss.fit_transform(songs)
```
...
```{python}
songs['cluster'] =  pipeline.predict(songs_matrix)

songs.groupby('cluster').mean()
```
...
```{python}
songs_full = pd.read_csv("https://www.dropbox.com/s/5ke5fi3hlu0f02w/top_artists_spotify.csv?dl=1")
```
...
```{python}
songs_full['cluster'] = pipeline.predict(songs_matrix) 

songs_full['cluster', 'Artist'].value_counts()
```
...
```{python}
top = [.28, .65, 21866, .55, 0, .008, .188, -10.2, .64, .21, 117, 3.9, .47]
...
means = songs.mean(axis = 1)
sds = songs.sd(axis = 1)

top = (top - means)/sds

sum(top * pipeline['pca'].components_[1])
sum(top * pipeline['pca'].components_[2])
```
...
```{python}
top = pd.read_csv("https://www.dropbox.com/s/t27qx7kvnylhirj/twenty-one-pilots.csv?dl=1")
```
